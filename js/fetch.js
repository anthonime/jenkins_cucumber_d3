...
function fetchBigJenkinsJson(config, callback, progressCallback, errorCallback) {
	...
	var url = config.jenkinsUrl + JSON_API + "?" 
				+ depth(1) 
				+ "&" + (config.debug?PARAM_PRETTY:"")
					+ "&" + PARAM_BUILD_TREE;
	progressCallback({message: "fetch started..."});
	console.log("Fetching main json from Jenkins API at ",url)
	fetchJson(url, function(data) {
		progressCallback({message: "got main JSON"});
		...
		for(var jIdx in jobs){
			...
			for(var cIdx in job.activeConfigurations){
				...
				for(var bIdx in config.builds){
					...
					var hasArtifacts = fetchBuildArtifacts(job,config,build,function(d){
						//when everything is fetched, return the jobs
						 if(pendingFetches==0){
							 progressCallback({message: "All jobs fetched ! "});
							 callback(jobs);
						 }
					}, function(error){
						console.log("ERROR for " + createKey(job,config,build) + " ", error);
					},progressCallback);
					if(hasArtifacts){
...
function createKey(job,config,build,relativePath){
	return job.name + "_" + config.name + "_" + build.number + "_" + relativePath;
}

function fetchBuildArtifacts(job, config, build, callback, errorCallback, progressCallback){
	if (build.artifacts) {
		...
		for ( var artifactIdx in build.artifacts) {
			...
			if (artifact.fileName && artifact.fileName.match(".*\\.json")) {
				...
				if(artifactValue.status!="loading"){
					...
					artifactValue.url = build.url + "artifact/" + artifact.relativePath;
					totalFetches++;
					pendingFetches++;
					fetchJson(artifactValue.url,function(d){
						alreadyFetched++;
						pendingFetches--;
						progressCallback({message: "Fetching cucumber JSON files...", alreadyFetched: alreadyFetched, totalFetches:totalFetches,pendingFetches:pendingFetches});
